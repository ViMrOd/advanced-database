...
def get_create_kinds():
    '''Get create for kinds'''
    return render_template("kind_create.html")     

@app.route("/kind/create", methods=["POST"])
def post_create_kinds():
    '''Post create for kinds'''
    data = dict(request.form)
    database.create_kind(data)
    return redirect(url_for("get_kind"))  
