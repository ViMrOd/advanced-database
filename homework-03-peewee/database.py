...
def create_pet(data):
    #print(data.keys())
    pet = Pet.create(name=data["name"], age=data["age"], owner=data["owner"], kind=data["kind_id"])

def create_kind(data):
    kind = Kind.create(kind_name=data["name"], food=data["food"], noise=data["sound"])

def get_pets():
    pets = Pet.select().join(Kind)
    pets_list = [pet.__data__.copy() for pet in pets]
    return pets_list
    #return list(pets)
...
def get_kinds():
    kinds = Kind.select()
    kinds_list = [kind.__data__.copy() for kind in kinds]
    return kinds_list
...
def update_pet(id, data):
    print(f'Pet: {data}')
    Pet.update(name=data["name"], age=data["age"], owner=data["owner"], kind=data["kind_id"]).where(Pet.id == id).execute()   

def update_kind(id, data):
    Kind.update(kind_name=data["name"], food=data["food"], noise=data["sound"]).where(Kind.id == id).execute()
