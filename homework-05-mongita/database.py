...
def retrieve_pets():
    pets_collection = pets_db.pets_collection
    # Use an aggregation pipeline to join pets and kinds collections
    pipeline = [
        {
            "$lookup": {
        ...
        {
            "$project": {
                "id": {"$toString": "$_id"},  # Convert _id to string for id
                "kind_name": "$kind_info.kind_name",
                "noise": "$kind_info.noise",
                "food": "$kind_info.food",
                # Add other pet fields you want to return
                # For example: "name": 1, "age": 1, etc.
            }
        }
    ]

    pets = list(pets_db.pets_collection.aggregate(pipeline))
    return pets
'''
